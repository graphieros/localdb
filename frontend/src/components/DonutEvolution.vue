<template>
  <div style="width: 100%">
      ...
      >
          <div>
              <input 
                  id="donutEvolutionCheckboxTooltip" 
                  type="checkbox" 
                  v-model="showTooltip" 
                  name="donutEvolutionCheckboxTooltip" 
                  :checked="showTooltip"
              >
              <label for="donutEvolutionCheckboxTooltip">{{ controlTranslations.showTooltip }}</label>
          </div>
          <div>
              <input 
                  ...
                  :checked="showLines"
                  @input="toggleLinesVisibility"
              >
            <label for="donutEvolutionCheckboxLine">{{ controlTranslations.showLines }}</label> 
          </div>
      </div>
      ...
      <svg 
              ...
              >
                  <div class="tonality-legend">
                      ...
                      >
                          ...
                          />
                          <span :style="`font-size:${maxFontSize * 0.9}px;`">
                              {{ tonality.name }}
                          </span>
                      </div>
                  </div>
           ...
           <g v-if="!showLineToggle">
              <g v-for="(donutSerie, i) in datasetCopy" :key="`yLabel_${i}`">
                  ...
                  >
                      <div 
                          class="y-label" 
                          :style="`font-size:${maxFontSize * 0.9}px; font-weight:${selectedSerieIndex === i ? '700' : '400'};`"
                      >
                          {{ donutSerie.name }}
                      </div>
                  </foreignObject>
          ...
          <g v-if="showLineToggle">
              <g 
                  ...
                  :key="`topic_${i}`" 
                  :style="selectedSerieIndex === i ? 'opacity:1' : selectedDonut.hasOwnProperty('x') ? 'opacity: 0.2' : 'opacity:1'" 
              >
              <!-- Yaxis Labels & checkboxes for line toggles -->
                  <foreignObject 
                      style="overflow: visible;" 
                      :x="0" 
                  ...
                  >
                      <label :for="topic.id"
                          class="y-label" 
                          :style="`cursor: pointer; font-size:${maxFontSize * 0.9}px; font-weight:${selectedPeriodIndex && selectedSerieIndex === i ? '700' : '400'}; `"
                      >
                          {{ topic.name }}
                      </label>
                      <input 
                          ...
                          @input="checkLine(i)"
                          :checked="visibleLines.includes(i)"
                      >
                  </foreignObject>
                  ...
                  >
                      ...
                      >
                          ...
                          <!-- visible line -->
                              ...
                              >
                                  {{ 
                                      Math.round(line.base).toLocaleString()
                                  }}
                              </text>
                          </g>
                      </svg>
          ...
          <g class="donuts-chart">
              ...
              >
                  ...
                  <!-- Horizontal line passing through all donuts of a serie -->
                      ...
                      <g v-if="selectedDonut.hasOwnProperty('x')">
                      ...
                      >
                          ...
                          <path
                              v-if="j < buildSeries(donutSerie).length -1 && showInnerEvolutionPath && !selectedDonut.hasOwnProperty('x') && donut.base && buildSeries(donutSerie)[j+1].base"
                          ...
                          <g 
                              class="pointer" 
                              @click="selectDonut({donut, serieIndex: i, x:calcXPosition(j) + calcHalfDonutSize(), y:calcYPosition(i + 1), id:donut.donutId, periodIndex:j, date:xLabels[j]})" 
                              @pointerover="hoverDonut({periodIndex:j})"
                              @pointerout="isTooltip = false; hoverPeriodIndex = undefined"
                          >
                              <!-- Hollow -->
                              ...
                              >
                                  {{ 
                                      Math.round(sum(donut.series)).toLocaleString()
                                  }}
                              </text>
              ...
              >
                  ...
                  >
                      ...
                      <span style="color: black; font-size: 1.3em;">
                          {{ xLabels[selectedPeriodIndex].display }}
                          <br/>
                          {{ sum(selectedDonut.series).toFixed(0).toLocaleString() }}
                      </span>
                      </span>
                  </div>
              </foreignObject>
 ...
 name: "DonutEvolution",
      ...
      datasetCopy(){
          ...
          return this.dataset.map((datapoint) => {
              return {
                  ...
                  total: datapoint.series.flatMap((serie) => {
                      return serie.values
                  }).reduce((a,b) => a + b, 0)
              }
  ...
  methods: {
      ...
      createLinePlot(line, topic){
        const max = Math.max(...topic.map((t) => t.base)) || Number.MIN_VALUE;
      ...
      hoverBetweenDonuts(topicIndex, periodIndex, donut1, donut2){
          ...
          let html = "";
          const monthPrevious = this.xLabels[periodIndex].display;
          const monthNext = this.xLabels[periodIndex + 1].display;
          ...
          if(isMultipleTonalities){
              html += `<b>${isFinite(this.calcEvolPercent(donut2.base, donut1.base)) ? this.calcEvolPercent(donut2.base, donut1.base) > 0 ? '+' : '' : " "}`;
              html += `${isFinite(this.calcEvolPercent(donut2.base, donut1.base)) ? this.calcEvolPercent(donut2.base, donut1.base).toFixed(1) : '-'}%</b>`;
          }
          ...
          donut1Serie.forEach((serie,i) => {
              ...
              <div style="display: flex; flex-direction: row; gap: 6px;">
                  ...
                  <span>
                  ${isFinite(this.calcEvolPercent(donut2Serie[i].value, serie.value)) ? this.calcEvolPercent(donut2Serie[i].value, serie.value) > 0 ? '+' : '-' : '-'}
                  ${isNaN(this.calcEvolPercent(donut2Serie[i].value, serie.value)) || !isFinite(this.calcEvolPercent(donut2Serie[i].value, serie.value)) ? '' : (Math.abs(this.calcEvolPercent(donut2Serie[i].value, serie.value))).toFixed(1)}%
                  </span>
              </div>
              `;
          })
      ...
      hoverDonut({periodIndex}){
          ...
          html += `
              <div style="grid-column: 1/span${this.tooltipGrid}; width: 100%;">
                  <b>${this.xLabels[periodIndex].display}</b>
                  <svg width="90px" viewBox="0 0 100 10" style="border-radius: 6px; margin-left: 6px;">${markers}</svg>
                  <br>
                  <b>Total: ${Math.round(sumOfAllAtPeriod).toLocaleString()}</b>
              </div>
              <div style="grid-column: 1/span${this.tooltipGrid}; width: 100%;">
          ...
          selectedPeriodData.forEach(serie => {
              ...
              html += `
                  <div>
                      ...
                      <b>${serie.value ? Math.round(serie.value).toLocaleString() : 0}</b>
      ...
      selectDonut({ donut, x, y, id, serieIndex = null, periodIndex = null, date = null }) {
          if(donut && donut.base === 0) return;
          // A datapoint with 0 or the same datapoint is selected twice:
          if(!donut || donut.base === 0){
              this.selectedSerieIndex = null;
              this.selectedPeriodIndex = null;
              this.selectedDonut = {};
              // emit false values to detect deselection
              this.$emit("selectDonut", {donut: false, periodEnd: false, periodStart: false});
              return;
          }
          // A new datapoint with data is selected:
          if(donut && donut.base > 0){
              this.$emit("selectDonut", {donut, periodEnd: date.periodEnd, periodStart: date.periodStart});
