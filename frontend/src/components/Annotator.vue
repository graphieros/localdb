			...
			case shape && shape.type === "line":
			  return `
				<g id="${shape.id}">
				  <path 
					id="${shape.id}" 
					d="${shape.path.replaceAll('.', '0.')}" 
					style="stroke:${shape.color + shape.alpha} !important; fill:none; stroke-width:${shape.strokeWidth} !important; stroke-linecap: round !important; stroke-linejoin: round !important;"        
				  />
				</g>
			  `;
		...
		this.$nextTick(() => {
		  switch (true) {
			...
			case this.activeShape === "line":
			  this.shapes.at(-1).path += ` ${this.pointerPosition.x} ${this.pointerPosition.y} `;
			  break;
		...
		switch (true) {
		  ...
		  case this.activeShape === "line":
			this.shapes.push({
			  ...
			  strokeWidth: this.copy(Math.abs(this.strokeSize)),
			  isDash: this.copy(this.isDash),
			  path: `M${this.pointerPosition.x} ${this.pointerPosition.y} Q `
			});
			this.lastSelectedShape = this.shapes.at(-1);
			break;
