<template>
  <div class="mini-score-chart">
        ...
        <g v-if="isSingle">
            <!-- Title -->
            <foreignObject style="overflow: visible;" :x="0" :y="10" width="100%" height="1em">
                <div class="title" :style="`font-family:${fontFamily} !important;`">
                    <!-- Titles for 'score-only & nps-only' use the name of the datapoint -->
                    <span v-if="isChartOfType('score-only') || isChartOfType('nps-only')">
                ...
                <foreignObject :x="0" :y="35" height="100" width="100%">
                    <div>
                        <b class="score">
                            ...
                            <span v-if="data.dataset[0].kpi.showRatio" class="kpi-ratio">
                                ({{ data.dataset[0].kpi.ratio}})
                            </span>
                        </b>
                    </div>
                    <div class="kpi-comment" v-if="data.dataset[0].kpi.hasTarget">
                        {{ data.dataset[0].kpi.targetTranslation }} : 
                        {{ data.dataset[0].kpi.target }}
                        <svg width="8" height="8" viewBox="0 0 12 12" fill="none" style="margin-bottom: -2px">
                            <g v-if="data.dataset[0].kpi.isOnTarget">
                                <path
                                    d="M5.84187 3.51297L5.93387 2.56536C5.97027 2.18456 6.32588 1.92616 6.68508 2.01896C7.22549 2.15896 7.60509 2.66096 7.60509 3.23657V4.45058C7.60509 4.72058 7.60509 4.85578 7.66349 4.95459C7.69669 5.01099 7.74229 5.05859 7.79669 5.09259C7.89269 5.15299 8.02349 5.15299 8.2847 5.15299H8.4431C9.12431 5.15299 9.46471 5.15299 9.67431 5.30899C9.83151 5.42619 9.94231 5.59779 9.98631 5.79219C10.0447 6.0522 9.91391 6.3774 9.65191 7.02701L9.52151 7.35101C9.44587 7.53866 9.41492 7.74134 9.43111 7.94302C9.52391 9.09263 8.6123 10.061 7.49789 9.99744L3.32825 9.75783C2.87264 9.73183 2.64504 9.71863 2.43944 9.53663C2.23344 9.35463 2.19463 9.16703 2.11743 8.79222C1.95313 7.99486 1.96063 7.17164 2.13943 6.3774C2.25264 5.87819 2.72944 5.60939 3.22185 5.67339C4.52746 5.84139 5.71107 4.86578 5.84187 3.51337V3.51297Z"
                                    stroke="#15B300"
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                />
                                <path
                                    d="M3.99985 5.80005L3.94505 5.98285C3.59029 7.16546 3.60943 8.42877 3.99985 9.60009"
                                    stroke="#15B300"
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                />
            ...
            <g v-if="isChartOfType('ranking-only')">
                <!-- Ranking & base -->
                <foreignObject :x="0" :y="35" height="100" width="100%">
                    <div :style="`color:${colors.tonalities[getProgressName(data.dataset[0].progression)]} !important`">
                        <b class="score">
                    ...
                    <div class="ranking-progression-comment">
                        <b :style="`color:${colors.tonalities[getProgressName(data.dataset[0].progression)]} !important`">
            ...
            <g v-if="isChartOfType('nps-only')">
                <!-- NPS Score -->
                <foreignObject :x="0" :y="35" height="100" width="100%">
                    <div :style="`color:${getNpsColor(data.dataset[0].score)}`">
                        <b class="score">
                            {{ data.dataset[0].score >= 0 ? `+${data.dataset[0].score.toLocaleString()}` : data.dataset[0].score.toLocaleString() }}
                        </b>
                    </div>
                </foreignObject>
            </g>
            ...
            <g v-if="isChartOfType('score-only')">
                ...
                <foreignObject :x="0" :y="30" height="100" width="100%">
                    ...
                    <svg height="24" viewBox="0 0 29 29">
                        <path :stroke="getScoreColor(data.dataset[0].score)" stroke-width="2" stroke-linecap="round" :d="smileys[getTonalityName(data.dataset[0].score)]" fill="none"/>
                    </svg>
                </foreignObject>
            </g>
            ...
            <g v-if="isChartOfType('score-tonality')">
                ...
                <!-- Bar chart -->
                <g v-for="(tonality, i) in createTonalityBarChart(data.dataset[0].tonalities)"
                ...
                <foreignObject :x="0" :y="80" height="32" width="100%">
                    <div class="legend__wrapper">
                        <div class="legend__item" v-for="(tonality, i) in formatTonalities(data.dataset[0].tonalities)" :key="`legend_tonality_${i}`">
        ...
        <template v-else>
            ...
            <g v-if="isChartOfType('nps-only')">
                <g v-for="(datapoint,i) in data.dataset" :key="`datapoint_${i}`">
                    <!-- Title -->
                    <foreignObject style="overflow: visible; padding: 0 5px" :x="i === 0 ? 0 : centerX" :y="10" width="50%" height="1em">
                        <div class="title" :style="`font-family:${fontFamily} !important;`">
                    ...
                    <!-- NPS Score -->
                    <foreignObject :x="i === 0 ? 0 : centerX" :y="35" height="100" width="50%">
                        <div :style="`color:${getNpsColor(datapoint.score)}`">
                            <b class="score">
                                {{ datapoint.score >= 0 ? `+${datapoint.score.toLocaleString()}` : datapoint.score.toLocaleString() }}
                            </b>
                        </div>
                    </foreignObject>
                </g>
                <line :x1="centerX" :x2="centerX" :y1="10" :y2="height - 10" stroke-width="0.6" stroke="#E1E5E8" stroke-linecap="round"/>
            </g>
            ...
            <g v-if="isChartOfType('score-only')">
                <g v-for="(datapoint,i) in data.dataset" :key="`datapoint_${i}`">
                    ...
                    <!-- Score & base -->
                    <foreignObject :x="i === 0 ? 0 : centerX" :y="30" height="100" width="50%">
                        <div :style="`color:${getScoreColor(datapoint.score)}`">
                            <b class="score">
                        ...
                        <svg height="24" viewBox="0 0 29 29">
                            <path :stroke="getScoreColor(datapoint.score)" stroke-width="2" stroke-linecap="round" :d="smileys[getTonalityName(datapoint.score)]" fill="none"/>
                        </svg>
                    </foreignObject>
                </g>
                <line :x1="centerX" :x2="centerX" :y1="10" :y2="height - 10" stroke-width="0.6" stroke="#E1E5E8" stroke-linecap="round"/>
            ...
            <g v-if="isChartOfType('score-tonality')">
                ...
                <!-- Bar charts -->
                <g v-for="(datapoint,i) in data.dataset" :key="`datapoint_${i}`">
                    <g v-for="(tonality, j) in createTonalityBarChart(datapoint.tonalities)"
                    ...
                    <rect 
                        ...
                        height="5"
                        @pointerover="tooltipActive = true; createTooltip(datapoint)"
