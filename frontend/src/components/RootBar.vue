<template>
    <svg 
      class="rootbar" 
      width="100%" 
      :style="`max-width:${maxWidth}px; font-family:${fontFamily}; padding-right: ${containerWidth / 16}px`" 
      :viewBox="`0 0 ${width} ${height}`"
  >
      <!-- CONNECTING PATHS BETWEEN PARENT CIRCLES & CHILDREN BARS -->
      <g class="rootbar__connexions">
          <g v-for="(path,i) in bars" 
              :key="`path_${i}`" 
              :style="styleBarOpacity(path, i, 0)"
          >
              <path v-for="(el,k) in Math.round(barWidth)" 
              :key="`el_${k}`"
              :d="`M
                  ${path.x1},${path.y1 - barWidth / 2 + k}
                  C${path.x1 - width / 20},${path.y1 - barWidth / 2 + k} 
                  ${path.x1 - width / 10},${path.y1 - barWidth / 2 + k} 
                  ${width / circlesPositionRatioX}, ${getParent(path).optY}
              `"
              :stroke="k === 0 || k === Math.round(barWidth) ? 'white' : path.color"
              :stroke-width="2"
              fill="none"
              :shape-rendering="k === 0 || k === Math.round(barWidth) - 1 ? 'auto' : 'crispEdges'"
              :opacity="0.15"
              />
               <!-- CHILDREN DONUT (SELECTED STATE) LEGEND -->
              <foreignObject v-if="showTonality && path.hasOwnProperty('series') && path.series.length && selectedBarIndex !== undefined"
                  class="bar-label" 
                  @click="selectBar(path, i)"  
                  :x="path.x2" 
                  :y="path.y1 + 100" 
                  :height="64"
                  :width="width / 3"
              >
                  <div :style="`font-size:${maxFontSize * 1.5}px; width: 100%; display: flex; align-items:center;
                  justify-content:center; background: white; color:#2d353c;`">
                      <b>{{ path.name }}</b>
                  </div>
                  <div class="rootbar__donut__legend" 
                      :style="`opacity:${path.id === selectedBar.id ? 1 : 0}`"
                  >
                      <div v-for="(tonality,k) in path.series" 
                          :key="`tonality_legend_${k}`" 
                          :style="`display:flex; flex-direction: column; font-size:${maxFontSize}px`"
                          class="rootbar__donut__legend__item"
                      >
                          <strong 
                              :style="`color:${tonality.color}; font-size: 1.6em;`"
                          >
                              {{ tonality.value.toFixed(0).toLocaleString() }}
                          </strong>
                          <span style="max-width:100px; line-height:18px; font-size:1.4em; color:#2d353c;">
                              {{ tonality.name }}
                          </span>
                      </div>
                  </div>
              </foreignObject>
              <!-- CHILDREN BAR LABEL & DONUT (if showTonality is set to true) -->
              <foreignObject 
                  class="bar-label" 
                  @click="selectBar(path, i)"  
                  :x="path.x2 + 10" 
                  :y="path.y1 - (height / (bars.length) / 2)" 
                  :height="height / (bars.length)"
                  :width="width / 3"
                  :style="subtopicHasTonalityData(path) ? 'cursor: pointer;' : 'cursor: initial;'"
              >
                  <div :style="`width: fit-content; display: flex; flex-direction: row; align-items:center;
                  justify-content:start; font-size:${maxFontSize}px; transition: unset !important;`">
                      <!-- CHILDREN BAR LABEL -->
                      <div :style="styleBarLabelEllipsis(i, 250)">
                          <template v-if="selectedParent.hasOwnProperty('data')">
                              <span 
                                  :style="`color:${getParent(path).color}; margin-right: 3px;`"
                              >
                                  <b>{{ isNaN(path.data / getParent(path).data * 100) ? 0 : (path.data / getParent(path).data * 100).toFixed(1) }}%</b>
                              </span>
                              <span style="color: black;">
                                  (&nbsp;{{ path.data.toFixed(0).toLocaleString() }}&nbsp;)&nbsp;{{ path.name }} 
                              </span>
                          </template>
                          <template v-else>
                              <span 
                                  style="margin-right:3px;"
                              >
                                  <b>{{ isNaN(path.data / totalOccurences * 100) ? 0 : (path.data / totalOccurences * 100).toFixed(1) }}%</b>
                                  <br v-if="selectedBarIndex !== undefined" style="line-height:25px"/>
                                  <span style="color: black;">
                                     (&nbsp;{{ path.data.toFixed(0).toLocaleString() }}&nbsp;)&nbsp;<span v-if="selectedBarIndex === undefined">{{ path.name }}</span>
                                  </span>
                              </span>
                          </template>
                      </div>
                      <!-- CHILDREN DONUT & MARKERS -->
                      <svg 
                          v-if="showTonality && path.hasOwnProperty('series') && subtopicHasTonalityData(path)"
                          :style="`
                              background: transparent; 
                              padding:0 !important; 
                              margin-left:${isNormalSizeDonut(i) ? '24px' : '-80px'};
                          `" 
                          :height="styleSvgSize(i)" 
                          :viewBox="`0 0 100 100`"
                      >
                          <!-- DONUT --> 
                          <path
                              style="fill:none" 
                              v-for="(arc, k) in isNormalSizeDonut(i) ? makeDonut(path, 50, 50, 100, 100) : makeDonut(path, 50, 50, 150, 150)" 
                              :key="`arc_${i}_${k}`" 
                              :d="arc.path" 
                              :stroke="arc.ratio < 0.01 ? 'transparent' : arc.color" 
                              :stroke-width="selectedBarIndex !== undefined ? 100 : 80"
                          />
                          <!-- MARKERS -->
                          <template v-if="selectedBarIndex === i && showTonality ">
                              <foreignObject
                                  v-for="(arc, k) in makeDonut(
                                  path,
                                  0,
                                  0,
                                  110,
                                  110
                                  )"
                                  :key="`arc_select_${k}`"
                                  :x="arc.center.endX"
                                  :y="arc.center.endY"
                                  height="100%"
                                  width="100%"
                                  style="overflow: visible"
                                  :id="`marker_${k}`"
                              >
                                      <div
                                      v-if="arc.proportion * 100 > 5"
                                      :style="`
                                          align-items:center; 
                                          background:white; 
                                          border-radius: 50%; 
                                          box-shadow: 0 3px 6px rgba(0,0,0,0.21);
                                          color: black; 
                                          height: 120px;
                                          width:120px;
                                          display: block; 
                                          display: flex; 
                                          font-size:3em; 
                                          font-weight: bold;  
                                          justify-content:center; 
                                          padding: 2px; 
                                          z-index:100;
                                      `"
                                      >
                                      <span style="line-height: 5px;">
                                          {{ (arc.proportion * 100).toFixed(0) }}%
                                          <br>
                                      </span>
                                  </div>
                              </foreignObject>
                       </template>
                      </svg>
                  </div>
              </foreignObject>
          </g>
      </g>
  
      <!-- PARENT CIRCLES & INFO -->
      <text v-if="showBase" :x="width / circlesPositionRatioX" y="-6" text-anchor="middle" font-size="20" font-weight="bold" fill="grey">
          Base: {{ totalOccurences.toFixed(0).toLocaleString() }}
      </text>
      <g v-for="(circle,i) in circles"  
          :key="`parent_${i}`" 
          @click="selectParent(circle)"
          class="rootbar__parent"
          :style="`cursor:${circle.data ? 'pointer' : 'default'};`"
          @pointerenter="styleHoverUnselected(circle)"
          @pointerleave="hoveredCircle = {}"
      >
          <circle 
              :cx="width / circlesPositionRatioX " :cy="circle.optY"
              :r="getCircleRadius(circle)"
              :fill="circle.color"
              :style="hoveredCircle.hasOwnProperty('id') && hoveredCircle.id===circle.id ? styleParentOpacity(circle,0.4) : styleParentOpacity(circle, 0.2)"
          />
          <!-- Circle label percentage -->
          <text 
              :x="width / circlesPositionRatioX" 
              :y="circle.optY - 5" 
              :font-size="`30px`" 
              :fill="styleTextColor(circle.color)"
              :style="hoveredCircle.hasOwnProperty('id') && hoveredCircle.id===circle.id ? styleParentOpacity(circle,0.4) : styleParentOpacity(circle, 0)" 
              text-anchor="middle"
              font-weight="bold"
          >
              {{ (circle.data / totalOccurences * 100).toFixed(1) }} %
        
          </text>
          <!-- Circle label base -->
          <text 
              :fill="styleTextColor(circle.color)"
              :font-size="`25px`" 
              :style="hoveredCircle.hasOwnProperty('id') && hoveredCircle.id===circle.id ? styleParentOpacity(circle,0.4) : styleParentOpacity(circle, 0)" 
              text-anchor="middle"
              :x="width / circlesPositionRatioX" 
              :y="circle.optY + 30" 
          >
              {{ circle.data.toFixed(0).toLocaleString() }}
        
          </text>
          <!-- PARENT names, minibar charts & legends -->
          <foreignObject 
              :style="`overflow: visible; cursor: ${circle.data ? 'pointer' : 'default'}; ${styleParentOpacity(circle, 0.2)}`"
              :height="getCircleRadius(circle) * 2" 
              width="200px"
              :x="0" 
              :y="circle.optY - getCircleRadius(circle)" 
          >
              <div 
                  class="rootbar__parent__label" 
                  :style="`border-left: 5px solid ${circle.color};background: linear-gradient(to right, ${circle.color}1f, transparent); position: relative; font-size:${maxFontSize * 1.35}px`"
              >
                  {{ circle.name }}
              <!-- Parent tonality bars (if showTonality prop is set to true) -->
                  <svg v-if="showTonality" :viewBox="`0 0 100 10`" class="sentiment">
                      <defs>
                          <clipPath id="round-corner">
                              <rect 
                                  height="10" 
                                  rx="5" 
                                  ry="5"
                                  width="100%" 
                                  x="0" 
                                  y="5" 
                              />
                          </clipPath>
                      </defs>
                      <g v-for="(tonality, k) in circle.tonalities"
                          :key="`tonality_selected_${k}`">
                          <rect 
                              :fill="getTonality(circle.name, circle.tonalities, k).color" 
                              :x="getTonality(circle.name, circle.tonalities, k).x1 ? getTonality(circle.name, circle.tonalities, k).x1 : 0.01" 
                              :y="5" 
                              :width="getTonality(circle.name, circle.tonalities, k).acc - getTonality(circle.name, circle.tonalities, k).x1 ? getTonality(circle.name, circle.tonalities, k).acc - getTonality(circle.name, circle.tonalities, k).x1 : 0.01" 
                              height="10"
                              :clip-path="k === 0 || k === circle.tonalities.length - 1 ? 'url(#round-corner)': ''"
                          /> 
                      </g>
                      <g class="minibar-white-border">
                          <rect rx="8" stroke-width="3" height="16" width="106" y="2.5" x="-3" stroke="#FFFFFF" fill="none"/>
                      </g>
                  </svg>
              </div>
          </foreignObject>
      </g>
  
      <!-- CHILDREN BARS -->
      <g class="rootbar__bars">
          <rect v-for="(bar,i) in bars" 
                :key="`bar_${i}`" 
                :x="bar.x1"  
                :y="bar.y1 - barWidth / 2" 
                :height="barWidth"
                :width="bar.x2 - bar.x1 ? bar.x2 - bar.x1 : 0.01" 
                :fill="bar.color" 
                @click="selectBar(bar, i)"
                :style="styleBarOpacity(bar, i, 0.1)"
                class="bar"
                rx="5"
                ry="5"
          />
      </g>
  
      <!-- VERTICAL SEPARATOR BETWEEN PARENT CIRCLES & CHILDREN BARS -->
      <g>
          <line 
                :x1="width / ratioSeparation" 
                :x2="width / ratioSeparation" 
                :y1="0" 
                :y2="height" 
                stroke="white" 
                stroke-width="3" 
          />
      </g>
  
      <!-- PARENT TONALITY LEGEND (visible if parent or bar is selected) -->
      <g v-if="showTonality && isTonalitySelected && (selectedBar.hasOwnProperty('id') || selectedParent.hasOwnProperty('id'))" @click="selectedParent = {}">
          <foreignObject 
                class="parent-legend" 
                :x="0" 
                :y="selectedTonality.optY + 92" 
                width="200px"
          >
              <div class="parent-legend__wrapper" :style="`font-size:${maxFontSize}px;`">
  
                  <div 
                      v-for="(tonality, k) in selectedTonality.parentTonalities" 
                      class="parent-legend__item" 
                      :key="`tonality_legend_${k}`"
                  >
                      <div :style="`background:${tonality.color}; height: 18px; width:18px; display: inline-block; border-radius: 100%`"></div> {{tonality.name}} : <b :style="`color:${tonality.color}`">{{isNaN(getTonality(selectedTonality.parentName, selectedTonality.parentTonalities,k).ratio) ? 0 : getTonality(selectedTonality.parentName, selectedTonality.parentTonalities,k).ratio.toFixed(1)}}%</b>; <b>{{tonality.value.toFixed(0).toLocaleString()}}</b>
                  </div>  
              </div> 
          </foreignObject>
      </g>
    </svg>
</template>
  
<script>
  /** EMIT METHODS:
   *  clicking on a circle : selectParent() emits "selectTopic" with the circle data to the parent component
   *  clicking on a bar : selectBar() emits "selectSubtopic" with the bar data to the parent component (also including its parent id)
   */
export default {
    name: "RootBar",
    props: {
        barFontSize: {
            type: String,
            default:"12px",
        },
        circleTextOffset: {
            type: Number,
            default: 5,
        },
        containerWidth: {
            type: Number,
            default: undefined,
        },
        dataset: {
            type: Array,
            default(){
                /** Dataset must respect this format:
                * 
                * [
                *      {
                *          id: String,
                *          name: String,
                *          color: String (hex format)
                *          children: [
                *              id: String,
                *              name: String,
                *              data: Number,
                *              series: [
                *                  {
                *                      name: String,
                *                      value: Number,
                *                      color: String (hex format)
                *                  }
                *              ]
                *          ]
                *      }
                * ]
                * 
                * IMPORTANT: 
                *  >>> IDs for both parents and children are mandatory
                * 
                *  >>> the 'series' property of the children is optional, 
                *      and will determine donut rendering,
                *      if the array is not empty AND if the showTonality component 
                *      prop is set to true. If one children has no 'series' prop or
                *      if it is empty, there will be no error, and simply no
                *      donut rendering for this datapoint.
                * 
                *  >>> the total base will be calculated from the sum of all
                *      children data property
                * 
                */
                return [
                    {
                        id: "01",
                        name: "Topic 1 with a very long label",
                        color: "#74cf77",
                        children: [
                            {
                                id: "01_01",
                                name: "Children label with a very long description that can display on multiple lines",
                                data: 150,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 120,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 59,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 102,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 36,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "01_02",
                                name: "Topic 1 Item 2",
                                data: 80,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 110150,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 59000,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 102000,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 2036,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "01_03",
                                name: "Topic 1 Item 3",
                                data: 122,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 37200,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 11595,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 2300,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 10036,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "01_04",
                                name: "Topic 1 Item 4",
                                data: 165,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 1125,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 256,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 1210,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 980,
                                        color: "#ebc034"
                                    }
                                ]
                                
                            },
                            {
                                id: "01_05",
                                name: "Topic 1 Item 5",
                                data: 25,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 10,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 15,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 20,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 46,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                        ]
                    },
                    {
                        id: "02",
                        name: "Topic 2",
                        color: "#6376DD",
                        children: [
                            {
                                id: "02_01",
                                name: "Topic 2 Item 1",
                                data: 120,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 12,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 4,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 9,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 6,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "02_02",
                                name: "Topic 2 Item 2",
                                data: 90,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 102,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 66,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 13,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 16,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "02_03",
                                name: "Topic 2 Item 3",
                                data: 102,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 200,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 12,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 12,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 3,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "02_04",
                                name: "Topic 2 Item 4",
                                data: 10,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 112,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 13,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 144,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 25,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "02_05",
                                name: "Topic 2 Item 5",
                                data: 16,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 2,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 1,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 10,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 2,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                        ]
                    },
                    {
                        id: "03",
                        name: "Topic 3",
                        color: "#f2bc27",
                        children: [
                            {
                                id: "03_01",
                                name: "Topic 3 Item 1",
                                data: 150,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 2,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 1,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 0,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 6,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "03_02",
                                name: "Topic 3 Item 2",
                                data: 125,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 36,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 12,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 15,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 25,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "03_03",
                                name: "Topic 3 Item 3",
                                data: 99,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 9,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 4,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 2,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 5,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "03_04",
                                name: "Topic 3 Item 4",
                                data: 9,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 4,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 2,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 7,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 10,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "03_05",
                                name: "Topic 3 Item 5",
                                data: 11,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 5,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 2,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 7,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 12,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                        ]
                    },
                    {
                        id: "04",
                        name: "Topic 4",
                        color: "#db621d",
                        children: [
                            {
                                id: "04_01",
                                name: "Topic 4 Item 1",
                                data: 34,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 3,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 2,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 3,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 2,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "04_02",
                                name: "Topic 4 Item 2",
                                data: 125,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 3,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 1,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 8,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 3,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "04_03",
                                name: "Topic 4 Item 3",
                                data: 99,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 2,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 3,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 4,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 1,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "04_04",
                                name: "Topic 4 Item 4",
                                data: 22,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 3,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 2,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 6,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 2,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "04_05",
                                name: "Topic 4 Item 5",
                                data: 29,
                                series: []
                            },
                        ]
                    },
                    {
                        id: "05",
                        name: "Topic 5",
                        color: "#732196",
                        children: [
                            {
                                id: "05_01",
                                name: "Topic 5 Item 1",
                                data: 24,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 12,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 3,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 1,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 4,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "05_02",
                                name: "Topic 5 Item 2",
                                data: 12,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 1,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 11,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 1,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 3,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "05_03",
                                name: "Topic 5 Item 3",
                                data: 9,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 3,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 3,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 15,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 10,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "05_04",
                                name: "Topic 5 Item 4",
                                data: 15,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 4,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 3,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 6,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 0,
                                        color: "#ebc034"
                                    }
                                ]
                            },
                            {
                                id: "05_05",
                                name: "Topic 5 Item 5",
                                data: 23,
                                series: [
                                    {
                                        name: "Positive",
                                        value: 9,
                                        color: "#15B300"
                                    },
                                    {
                                        name: "Neutral",
                                        value: 2,
                                        color: "#ccc"
                                    },
                                    {
                                        name: "Negative",
                                        value: 7,
                                        color: "#F17171"
                                    },
                                    {
                                        name: "Mixed",
                                        value: 6,
                                        color: "#ebc034"
                                    }
                                ]
                            }
                        ]
                    },
                ]
            }
        },
        fontFamily: {
            type: String,
            default: "Product Sans"
        },
        maxWidth: {
            type: Number,
            default: 1500,
        },
        showBase: {
            type: Boolean,
            default: false,
        },
        showTonality: {
            type: Boolean,
            default: false,
        },
        width: {
            type: Number,
            default: 1200,
        }
    },
    data() {
        return {
            defaultColors: ["#74cf77","#6376DD","#f2bc27","#db621d","#732196","#b30000", "#0d88e6", "#00b7c7", "#5ad45a", "#ebdc78", "#fd7f6f", "#7eb0d5", "#b2e061", "#bd7ebe", "#ffb55a", "#ffee65", "#beb9db", "#fdcce5", "#8bd3c7"],
            gap: 3,
            height: this.dataset.flatMap((category) => {
                return category.children;
            }).length * 50 + this.dataset.length * 50,
            hoveredCircle: {},
            isTonalitySelected: false,
            circlesPositionRatioX: 4.5,
            maxFontSize: this.containerWidth ? this.containerWidth * 0.02 : 15,
            ratioSeparation: 2.3,
            parentElementWidth: this.containerWidth ? this.containerWidth : this.maxWidth,
            selectedBar: {},
            selectedBarIndex: undefined,
            selectedParent: {},
            selectedTonality: {},
        };
    },
    computed: {
        barWidth(){
            return (this.height - this.children.length * this.gap) / this.children.length;
        },
        children(){
            return this.formattedDataset.flatMap((category) => {
                return category.children.map((child) => {
                    return {
                        ...child,
                        parentId: category.id,
                        parentName: category.name,
                        color: category.color,
                    }
                })
            }).sort((a,b) => b.data - a.data)
        },
        bars(){
            return this.children.map((child,i) => {
                return {
                    ...child,
                    y1: i * this.barWidth + this.barWidth / 2 + this.gap * i,
                    y2: i * this.barWidth + this.barWidth / 2 + this.gap * i,
                    x1: this.width / this.ratioSeparation,
                    x2: this.width / this.ratioSeparation + (child.data / this.maxBar) * this.width / 3,
                }
            }).sort((a,b) => a.data - b.data)
        },
        circles(){
            const parents = [...this.formattedDataset].map((parent) => {
                const {id, name, color, children} = parent;
                let data = parent.children.map((child) => {
                    return child.data
                }).reduce((a, b) => a + b, 0);
                return {
                    id, name, data, color, children
                }
            }).sort((a,b) => b.data - a.data);
            return parents.map((parent, i) => {
                const allY = this.bars.filter((bar) => bar.parentId === parent.id).map((child) => {
                        return child.y1;
                    });
                    const max = Math.max(...allY);
                    const min = Math.min(...allY);
                const optY = (this.height / parents.length) * i + this.height/parents.length / 2;
                const y = min + (max - min) / 2;
                // DISPLAY OF MAIN TOPICS TONALITIES
                let tonalities = parent.children.flatMap((child) => {
                    if(child.series.length > 0){
                        return child.series.map((serie) => {
                            return {
                                name: serie.name,
                                color: serie.color
                            }
                        });
                        // To always display tonalities with the same ordering, append the following: .sort((a,b) => a.name.localeCompare(b.name)).reverse()
                    }
                });
                tonalities = [...new Map(tonalities.map(v => [v && v.name ? v.name : "", v])).values()].filter((el) => el !== undefined);
                tonalities = tonalities.map((tonality) => {
                    return {
                        ...tonality,
                        value: parent.children.flatMap((child) => {
                            if(child.series.length > 0){
                                return child.series.map(serie => {
                                    if(serie.name === tonality.name){
                                        return serie.value
                                    }
                                })
                            }
                        })
                        .filter((el) => el !== undefined)
                        .reduce((a,b) => a + b, 0)
                    }
                });
                //////////////////////////////////////
                return {
                    ...parent,
                    y,
                    max,
                    min,
                    optY,
                    tonalities
                }
            });
        },
        formattedDataset(){
            return this.dataset.sort((a,b) => a.id - b.id).map((serie, i) => {
                return {
                    ...serie,
                    color: serie.color ? serie.color : this.defaultColors[i]
                }
            })
        },
        maxBar(){
            return Math.max(...this.formattedDataset.flatMap((category) => {
                return category.children.map((child) => child.data)
            }));
        },
        maxCircle(){
            const maxCircle = Math.max(...this.circles.map((category) => {
                return category.data
            }));
            return maxCircle;
        },
        totalOccurences(){
            return this.circles.map((parent) => parent.data).reduce((a, b) => a + b, 0);
        }
    },
    watch: {
        containerWidth: {
            handler(val){
                this.normalizeFontSize(val);
            }
        }
    },
    mounted(){
        this.normalizeFontSize(this.containerWidth);
    },
    methods: {
      // PARENTS TONALITY BARS
      getTonality(parentName, tonalities, index) {
        const total = tonalities.map((t) => t.value).reduce((a, b) => a + b, 0);
        let arr = [];
        let acc = 0;
        for(let i = 0; i < tonalities.length; i += 1){
          const t = tonalities[i];
          const x1 = acc;
          const x2 = t.value / total * 100;
          acc += x2;
          arr.push({...t, x1, x2, ratio:x2, total, acc, parentName})
        }
        return arr[index];
      },
      showPointedTonality(payload){
          const {parentName, parentTonalities, optY} = payload;
          this.isTonalitySelected = true;
          this.selectedTonality = {
              optY,
              parentName,
              parentTonalities
          };
      },
      // COLOR CONVERSION UTILS
      componentToHex(c) {
        let hex = c.toString(16);
        return hex.length == 1 ? "0" + hex : hex;
      },
      hexToRgb(hex) {
        const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
        return result
          ? {
              r: parseInt(result[1], 16),
              g: parseInt(result[2], 16),
              b: parseInt(result[3], 16),
            }
          : hex;
      },
      rgbToHex(color) {
        const elements = color.replace("rgb(", "").replace(")", "").split(",");
        const r = Number(elements[0]);
        const g = Number(elements[1]);
        const b = Number(elements[2]);
        return `#${this.componentToHex(r)}${this.componentToHex(
          g
        )}${this.componentToHex(b)}`;
      },
      // SELECTORS
      subtopicHasTonalityData(subtopic){
          const sumTonality = subtopic.series
              .map((serie) => serie.value)
              .reduce((a,b) => a + b, 0);
          return sumTonality > 0;
      },
      selectBar(bar, index){
          if(this.subtopicHasTonalityData(bar)){
              this.showPointedTonality({
              optY: this.circles.find((circle) => circle.name === bar.parentName).optY,
                  parentName: bar.parentName,
                  parentTonalities: this.circles.find((parent) => {
                      return parent.name === bar.parentName
                  }).tonalities
              })
              if(this.selectedBarIndex !== undefined && index === this.selectedBarIndex){
                  this.selectedBar = {};
                  this.selectedTonality = {};
                  this.selectedBarIndex = undefined;
              }else{
                  this.selectedBar = bar;
                  this.selectedBarIndex = index;
              }
              this.$emit("selectSubtopic", bar);
              this.selectedParent = {};
          }
      },
      selectParent(parent){
          if(parent.data){
              this.$emit("selectTopic", parent);
              this.showPointedTonality({
                  optY: parent.optY,
                  parentName: parent.name,
                  parentTonalities: parent.tonalities
              });
              this.selectedBarIndex = undefined;
              this.selectedBar = {};
              if(this.selectedParent.hasOwnProperty("id") && parent.id === this.selectedParent.id){
                  this.selectedParent = {};
                  this.selectedTonality = {};
              }else{
                  this.selectedParent = parent;
              }
          }
      },
      // GETTERS
      getCircleRadius(parent){
          if(this.selectedParent.id === parent.id){
              const sizeSelected = this.maxCircle / this.maxCircle * this.height / this.circles.length / 2;
              if(sizeSelected > 100){
                  return 100;
              }
              return sizeSelected;
          }else{
              const size = parent.data / this.maxCircle * this.height / this.circles.length / 2;
              if(size < 60){
                  return 60;
              }
              if(size > 100){
                  return 100;
              }
              return size;
          }
      },
      getParent(plot){
          return this.circles.find((circle) => circle.id === plot.parentId);
      },
      getParentLabelYPosition(parent, offset){
          if(this.selectedParent.id === parent.id){
              return parent.optY + this.maxCircle / this.maxCircle * offset;
          }else{
              return parent.optY + parent.data / this.maxCircle * offset;
          }
      },
      getParentLegendXPosition(parent){
          if(this.selectedParent.id === parent.id){
              return this.width / 6 - this.maxCircle / this.maxCircle * this.height / this.circles.length / 2 - 5;
          }else{
              return this.width / 6 - parent.data / this.maxCircle * this.height / this.circles.length / 2 - 5;
          }
      },
      // DONUT GENERATION METHODS
      addVector([a1, a2], [b1, b2]) {
        return [a1 + b1, a2 + b2];
      },
      createArc([cx, cy], [rx, ry], [position, ratio], phi) {
        ratio = ratio % (2 * Math.PI);
        const rotMatrix = this.rotateMatrix(phi);
        const [sX, sY] = this.addVector(
          this.matrixTimes(rotMatrix, [
            rx * Math.cos(position),
            ry * Math.sin(position),
          ]),
          [cx, cy]
        );
        const [eX, eY] = this.addVector(
          this.matrixTimes(rotMatrix, [
            rx * Math.cos(position + ratio),
            ry * Math.sin(position + ratio),
          ]),
          [cx, cy]
        );
        const fA = ratio > Math.PI ? 1 : 0;
        const fS = ratio > 0 ? 1 : 0;
        return {
          startX: sX,
          startY: sY,
          endX: eX,
          endY: eY,
          path: `M${sX} ${sY} A ${[
            rx,
            ry,
            (phi / (2 * Math.PI)) * 360,
            fA,
            fS,
            eX,
            eY,
          ].join(" ")}`,
        };
      },
      makeDonut(item, cx, cy, rx, ry) {
        let { series } = item;
        if(!series) return {...series, proportion:0, ratio:0, path:'', startX:0, startY:0,endX:0,center:{}};
        series = series.map((el) => {
          return {
              ...el,
              value: el.value === 0 ? 0.001 : el.value // if not this, donut doesn't show at all because the arc overflows 360°
          }
        });
        const sum = [...series]
          .map((serie) => serie.value)
          .reduce((a, b) => a + b, 0);
        const ratios = [];
        let acc = 0;
        for (let i = 0; i < series.length; i += 1) {
          const proportion = series[i].value / sum;
          const ratio = proportion * (Math.PI * 2);
          // midProportion & midRatio are used to find the midpoint of the arc to display markers
          const midProportion = series[i].value / 2 / sum;
          const midRatio = midProportion * (Math.PI * 2);
          const { startX, startY, endX, endY, path } = this.createArc(
            [cx, cy],
            [rx, ry],
            [acc, ratio],
            110
          );
          ratios.push({
            ...series[i],
            proportion,
            ratio,
            path,
            startX,
            startY,
            endX,
            endY,
            center: this.createArc(
              [cx, cy],
              [rx * 2, ry * 2],
              [acc, midRatio],
              110
            ), // center of the arc, to display the marker. rx & ry are larger to be displayed with a slight offset
          });
          acc += ratio;
        }
        return ratios;
      },
      matrixTimes([[a, b], [c, d]], [x, y]) {
        return [a * x + b * y, c * x + d * y];
      },
      rotateMatrix(x) {
        return [
          [Math.cos(x), -Math.sin(x)],
          [Math.sin(x), Math.cos(x)],
        ];
      },
      // CONDITIONAL STYLING METHODS
      isNormalSizeDonut(index){
          return (this.selectedBarIndex === undefined || this.selectedBarIndex !== index);
      },
      normalizeFontSize(containerSize){
          switch (true) {
                  case containerSize > 900:
                      this.maxFontSize = 13;
                      break;
  
                  case containerSize < 700:
                      this.maxFontSize = 17;
                      break;
              
                  default:
                      this.maxFontSize = 15;
                      break;
              }
      },
      styleBarLabelEllipsis(index, width){
          if(this.isNormalSizeDonut(index)){
              return `width: ${width}px; display: flex; align-items:center; text-align:left; font-size: 1.3em; line-height: 0.9em;`
          }else{
              return `width: ${width}px; display: flex; align-items:center; text-align:left; font-size: 1.3em; line-height: 0.9em`;
          }
      },
      styleBarOpacity(path, index, fadedValue){
          if(this.selectedBarIndex !== undefined){
              if(this.selectedBarIndex === index){
                  return 'opacity:1';
              }
              return `opacity: ${fadedValue}`;
          }else if(this.selectedParent.hasOwnProperty("id")){
              if(this.selectedParent.id === path.parentId){
                  return 'opacity:1';
              }
              return `opacity: ${fadedValue}`;
          }else{
              return 'opacity:1';
          }
      },
      styleHoverUnselected(parent){
          this.hoveredCircle = parent;
      },
      styleParentFontSize(parent, ratio){
          if(this.selectedParent.id === parent.id){
              return this.maxCircle / this.maxCircle * ratio;
          }else{
              return parent.data / this.maxCircle * ratio;
          }
      },
      styleParentOpacity(parent, min){
          if(this.selectedBarIndex !== undefined){
              if(this.selectedBar.parentId === parent.id){
                  return "opacity:1";
              }
              return `opacity:${min}`
          }else if(this.selectedParent.hasOwnProperty("id")){
              if(this.selectedParent.id === parent.id){
                  return "opacity:1";
              }
              return `opacity:${min}`;
          }else{
              return "opacity:1";
          }
      },
      styleSvgSize(index){
          if(this.isNormalSizeDonut(index)){
              const size = this.barWidth / 3;
              if(size > 21){
                  return 21;
              }
              return size;
          }else{
              return 42; // :)
          }
      },
      styleTextColor(bgColor) {
        // returns white or black text color based on the bgColor color shade 
        if (bgColor) {
          let color = bgColor;
          if(color.charAt(0) !== "#"){
              color = this.rgbToHex(bgColor);
          }
          color = color.substring(1, 7);
          let r = parseInt(color.substring(0, 2), 16);
          let g = parseInt(color.substring(2, 4), 16);
          let b = parseInt(color.substring(4, 6), 16);
          let uiColors = [r / 255, g / 255, b / 255];
          let c = uiColors.map((col) => {
            if (col <= 0.03928) {
              return col / 12.92;
            }
            return Math.pow((col + 0.055) / 1.055, 2.4);
          });
          let L = 0.2126 * c[0] + 0.7152 * c[1] + 0.0722 * c[2];
          return L > 0.3 ? "#000000" : "#FFFFFF";
        }
        return "#000000";
      },
    },
};
</script>
  
<style lang="scss" scoped>
  *{
      transition: all 0.15s ease-in-out;
  }
  svg {
      background: white;
      overflow: visible;
      padding: 24px 24px 24px 0;
      user-select: none;
  }
  svg.sentiment{
      background: transparent;
      height: 9px;
      margin-top: 6px;
      overflow: visible;
      padding: 0;
  }
  .bar-label{
      overflow: visible;
      div{
          align-items:center;
          color: black;
          display: flex;
          height: 100%;
          justify-content: start;
      }
  }
  .bar {
      cursor: pointer;
  }
  circle{
      stroke: white;
      stroke-width: 3;
  }
  .rootbar{
      &__donut{
          &__legend{
              align-items:center;
              background: white;
              border-radius: 12px;
              border-top: 1px solid grey;
              display: flex;
              flex-wrap: wrap;
              height: fit-content !important;
              justify-content: space-around !important;
              padding-top: 12px;
              width: 400px;
              &__item{
                  padding-bottom:6px;
              }
          }
      }
      &__parent{
          overflow: visible;
          &__label{
              align-items:start;
              border-radius: 3px;
              display: flex;
              flex-direction: column;
              font-size: 24px;
              font-weight: bold;
              height: 100%;
              justify-content: center;
              padding: 0 36px 0 12px;
              text-align:left;
              width:100%;
          }
      }
  }
  .parent-legend{
      height: 200px;
      overflow: visible;
      padding: 6px 0;
      &__item{
          display: block;
          font-size: 1.4em;
          font-weight: 400;
          height: fit-content;
          width: 100%;
      }
      &__wrapper{
          background: white;
          border-radius: 8px;
          box-shadow: 0 3px 6px rgba(0,0,0,0.11);
          cursor: pointer;
          height: fit-content;
          padding: 6px;
          width: 280px;
    }
}
</style>