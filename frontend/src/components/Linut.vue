        ...
        <svg class="linut" ref="svgLine" width="100%" :viewBox="`0 0 ${width} ${height}`" :style="`font-family:${fontFamily}`">
            <g class="linut__amenities">
                ...
                <g class="linut__yAxis">
                    ...
                    <text :x="xMargin - 6" :y="maxCoordinate + 2" text-anchor="end" :font-size="8" font-weight="bold">
                        {{ max }}
                    </text>
                    <!-- <text :x="xMargin - 6" :y="midCoordinate + 2" text-anchor="end" :font-size="8" font-weight="bold">
                        0
                    </text> -->
                    <text :x="xMargin - 6" :y="minCoordinate + 2" text-anchor="end" :font-size="8" font-weight="bold">
                        {{ min }}
                    </text>
...
export default {
  ...
  data() {
            ...
            return this.dataset.map((serie) => {
                return {
                    ...
                    data: serie.data.map((value, i) => {
                        return {
                            value,
                            x: ((this.width - this.xMargin) * i) / serie.data.length + this.xMargin,
                            y: this.normalize({
                                value,
    ...
    methods: {
        ...
        calcMax(serie){
            return Math.max(...serie.map((s) => {
                  return s.data.length;
                }));
        },
        getMax(serie){
            return Math.max(...serie.flatMap((s) => {
                return s.data;
            }));
        },
        getMin(serie){
            return Math.min(...serie.flatMap((s) => {
                return s.data;
              }))
            ;
        },
        ...
        showTooltip(index){
            ...
            let sumAtIndex = this.dataset.flatMap((serie) => {
            ...
            this.selectedSeries = this.dataset.map((serie) => {
            return {
                ...serie,
                data: serie.data[this.selectedIndex],
                ratio: Math.abs(serie.data[this.selectedIndex]) / sumAtIndex * 100
            }}).filter((_serie,i) => {
