<template>
  ...
  >
    ...
    <div
      ...
      :style="`align-items:${title.position}; justify-content:${title.position}`"
    >
      <div
        class="donut-wordcloud__title"
        v-if="showTitle"
        :style="`color:${title.color}; font-size:${title.fontSize}; font-weight:${title.fontWeight}`"
      >
        {{ title.label }}
      </div>
      <div
        ...
        :style="`color:${subtitle.color}; font-size:${subtitle.fontSize}`"
      >
        {{ subtitle.label }}
      </div>
    </div>
        ...
        >
          ...
          <div>
            ...
            <strong v-if="selectedDonutIndex !== undefined" >
              :
              <span :style="`color: ${circles[selectedDonutIndex].series[i].color}`">
                {{
    ...
    <div
      ...
      v-if="showLegend && individualLegend"
    >
    <div v-if="selectedDonutIndex === undefined" class="donut-wordcloud__legend__list">
        <div v-for="(legend, i) in sortedDataset[0].series" :key="`legend_placeholder_${i}`" style="color: transparent">
            &nbsp;
        </div>
    </div>
      <div class="donut-wordcloud__legend__list" v-else>
        <div
          class="donut-wordcloud__legend__item"
          v-for="(legend, i) in sortedDataset[selectedDonutIndex].series"
          :key="`legend_item_${i}`"
          :style="`${
            selectedLegend !== undefined
              ? selectedLegend === i
      ...
      >
        ...
        <template v-if="!justWords">
          <path
            ...
            "
            :style="`${selectedDonutIndex === i ? 'opacity: 0' : ''}; ${
              selectedLegend !== undefined
                ? selectedLegend === j
                  ? `opacity:1; stroke-width:${donutWidth * 1.618}`
                  : 'opacity: 0.15'
                : 'opacity:1'
            }`"
          />
        ...
        <template
          v-if="
            selectedDonutIndex === undefined && selectedLegend !== undefined
          "
        >
          <foreignObject
            id="markers"
...
export default {
      ...
      default() {
        return {
          items: [
            ...
            {
              icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#15B300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>',
              label: "Satisfait",
            },
            {
              icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C4C4C4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="8" y1="15" x2="16" y2="15"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>',
              label: "Neutre",
            },
            {
              icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#F17171" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M16 16s-1.5-2-4-2-4 2-4 2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>',
              label: "Insatisfait",
            },
          ],
        };
  ...
  methods: {
    ...
    placeOnFirstRow(circle) {
      if (this.isFirstCircleOfRow && !this.lastCircle.hasOwnProperty("x")) {
    ...
    placeAtCenterAndResizeViewBox(circle) {
      this.svgWidth =
        (circle.r * 2 + this.donutWidth * 2) * (this.maxItemsPerRow + 2);
    ...
    placeDonutOnTheRightFromLast(circle) {
      return (
        this.lastCircle.x + this.lastCircle.r + this.donutWidth * 2 + circle.r
      );
    },
